...
if(isAfterDeadline()){
    ...
    leaveMsg.innerText="Leave cannot be applied after 11:30 AM.";
    return;
}
...
function renderAttendanceApprovalTable(){
  ...
  // STEP 1: FILTER FIRST
  let filteredData = attApprovalDataFull.filter(row => {
  ...
  // STEP 2: PAGINATE FILTERED DATA
  let totalPages = Math.ceil(filteredData.length / attApprovalPageSize);
  if(totalPages === 0) totalPages = 1;

  if(attApprovalPage > totalPages){
    attApprovalPage = totalPages;
  }
  ...
  pageData.forEach((row) => {
    ...
    const originalIndex = attApprovalDataFull.indexOf(row);
    const sheetRow = originalIndex + 2;
    ...
    if(userRole === "BPM"){
      html += `<tr>
        ...
        <td>${row[3]}</td>
        <td>${row[7]}</td>
        <td><button onclick="approveAttendance(${sheetRow},'approveAttendanceBPM')">Approve</button></td>
      </tr>`;
    }
    ...
    if(userRole === "Block"){
      html += `<tr>
        ...
        <td>${row[3]}</td>
        <td>${row[7]}</td>
        <td><button onclick="approveAttendance(${sheetRow},'approveAttendanceBlock')">Final Approve</button></td>
      </tr>`;
    }
  ...
  html += `<tr>
    <td colspan="5">
      ...
      Page ${attApprovalPage} of ${totalPages}
...
function openCamera() {
	...
	if(isAfterDeadline()){
    alert("Attendance marking is closed after 11:30 AM.");
    return;
}
    ...
    liveTimeInterval = setInterval(() => {
        const now = new Date();
        overlay.innerHTML =
            `<b>EMP Name:</b> ${employeeName}<br>` +
            `<b>CLF | VO:</b> ${userBlock}<br>` +
            `<b>Date:</b> ${now.toLocaleDateString()}<br>` +
            `<b>Time:</b> ${now.toLocaleTimeString()}<br>` +
            `<b>Lat:</b> ${currentLatitude}<br>` +
            `<b>Long:</b> ${currentLongitude}<br>` +
...
doc.setFontSize(13);
doc.text("Attendance & Leave Monitoring System", pageWidth/2, 30, {align:"center"});
...
/* ===== PROCESS ATTENDANCE WITH ABSENT + LEAVE CHECK ===== */
...
attData.slice(1).forEach(row=>{
    employeeSet.add(row[1]);
});

/* Create attendance map */
attData.slice(1).forEach(row=>{
    let rowDate = formatDate(new Date(row[0]));
    let key = row[1] + "_" + rowDate;
    attendanceMap[key] = row;
});

/* Create leave map */
leaveData.slice(1).forEach(row=>{
    let leaveStart = new Date(row[3]);
    let leaveEnd = new Date(row[4]);

    for(let d = new Date(leaveStart); d <= leaveEnd; d.setDate(d.getDate()+1)){
        let key = row[1] + "_" + formatDate(d);
        leaveMap[key] = row;
    }
});
...
for(let d = new Date(from); d <= to; d.setDate(d.getDate()+1)){
    ...
    employeeSet.forEach(name=>{
        ...
        let key = name + "_" + formattedDate;

        if(attendanceMap[key]){

            let fullTimestamp = new Date(attendanceMap[key][0]);
            ...
            let monthKey = getMonthYear(currentDate);
            if(!monthlySummary[monthKey]) monthlySummary[monthKey] = {attendance:0, leave:0};
            monthlySummary[monthKey].attendance++;
   ...
   didParseCell: function (data) {
    ...
    if (data.column.index === 3) {
        ...
        if (data.cell.raw === "Present") {
            data.cell.styles.textColor = [16,163,74];
            data.cell.styles.fontStyle = "bold";
        }

        if (data.cell.raw === "On Leave") {
...
/* ================================
   DOWNLOAD EMPLOYEE MONTHLY ONLY
================================ */

async function downloadEmployeeMonthlyBreakdownPDF(){
...
doc.setFontSize(16);
doc.setFont(undefined,"bold");
doc.text("Attendance Monitoring System", pageWidth/2, 12, {align:"center"});

doc.setTextColor(0,0,0);
doc.setFontSize(12);
doc.text("Block: " + userBlock, pageWidth/2, 30, {align:"center"});
...
/* Create attendance map */
attData.slice(1).forEach(row=>{
    let date = formatDate(new Date(row[0]));
    attendanceMap[row[1] + "_" + date] = true;
});
...
for(let d = new Date(from); d <= to; d.setDate(d.getDate()+1)){
    ...
    let monthKey = getMonthYear(dateObj);

    employeeSet.forEach(name=>{

        if(!employeeMonthly[monthKey])
            employeeMonthly[monthKey] = {};

        if(!employeeMonthly[monthKey][name])
            employeeMonthly[monthKey][name] = {present:0, leave:0, absent:0};

        let key = name + "_" + formattedDate;

        if(attendanceMap[key])
            employeeMonthly[monthKey][name].present++;
        else if(leaveMap[key])
            employeeMonthly[monthKey][name].leave++;
        else
            employeeMonthly[monthKey][name].absent++;
    });
}
...
doc.setFontSize(15);
doc.setFont(undefined,"bold");
doc.text("EMPLOYEE MONTHLY BREAKDOWN", pageWidth/2, 45, {align:"center"});

let breakdownRows = [];

Object.keys(employeeMonthly).forEach(month=>{
    Object.keys(employeeMonthly[month]).forEach(emp=>{
        let data = employeeMonthly[month][emp];
        let workingDays = data.present + data.absent;

let percent = workingDays > 0
    ? ((data.present / workingDays) * 100).toFixed(1) + "%"
    : "0%";
...
doc.autoTable({
    startY: 55,
    head: [["Month","Employee","Present","Leave","Absent","Attendance %","Block Remarks"]],
    body: breakdownRows,
    theme: "grid",
    styles: { 
        fontSize: 9 
...
for(let i=1;i<=totalPages;i++){
    ...
    doc.text("Attendance System", 14, pageHeight - 10);
}

doc.save("Employee_Monthly_Breakdown_" + userBlock + ".pdf");
...
renderReportTable = function(leaveData){
	...
	const empBtn = document.createElement("button");
        empBtn.innerHTML = "<i class='fa-solid fa-chart-line'></i> Employee Monthly Breakdown";
        empBtn.style.marginLeft = "10px";
        empBtn.onclick = downloadEmployeeMonthlyBreakdownPDF;
...
function checkDailyDeadline(){
    ...
    if(isAfterDeadline()){
        ...
        const attBtn = document.querySelector("#attendanceTab button");
        if(attBtn){
            attBtn.disabled = true;
            attBtn.innerText = "Attendance Closed (After 11:30 AM)";
        }

        // Disable Leave Button
        const leaveBtn = document.querySelector("#leaveTab button");
        if(leaveBtn){
            leaveBtn.disabled = true;
            leaveBtn.innerText = "Leave Closed (After 11:30 AM)";
        }
